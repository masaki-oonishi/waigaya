...
def main():
    ...
    if status.level < 5:
        ...
        current_graphic = get_image_as_base64("resource/egg_evolution.png")
    else:
        current_graphic = get_image_as_base64("resource/bird.png")
    ...
    with st.sidebar:
        ...
        st.markdown(f"<div style='font-size: 11px; color: #666; margin-top: 2px;'>EXP: {status.current_exp} / {next_exp}</div>", unsafe_allow_html=True)
    ...
    if st.session_state.screen == "room":
        ...
        render_mojimoji_with_overlay(current_graphic, active_effect, height=300, key=f"room_{status.level}_{bool(active_effect)}")
    ...
    # 📖 【日記ルート】入力画面
        ...
        with col_d_top2:
            render_mojimoji_with_overlay(current_graphic, active_effect, height=180, key=f"diary_top_{status.level}_{bool(active_effect)}")
    ...
    # 💌 【日記ルート】演出画面
        ...
        with col_da_top2:
            render_mojimoji_with_overlay(current_graphic, active_effect, height=180, key=f"diary_anim_top_{status.level}_{bool(active_effect)}")
            st.markdown(f"<h4 style='text-align: center; margin-top: 5px; margin-bottom: 5px;'>🐾 Lv.{status.level}</h4>", unsafe_allow_html=True)
            next_exp = status.get_next_level_exp()
    ...
    # 💬 【会話ルート】LINE風トーク画面
        ...
        with col_chat_top2:
            render_mojimoji_with_overlay(current_graphic, active_effect, height=180, key=f"chat_top_{status.level}_{bool(active_effect)}")
            st.markdown(f"<h4 style='text-align: center; margin-top: 5px; margin-bottom: 5px;'>🐾 Lv.{status.level}</h4>", unsafe_allow_html=True)
            next_exp = status.get_next_level_exp()
